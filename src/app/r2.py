...
def _key(member_id: int) -> str:
    ...
    return f"cards/{member_id}_{month}.png"


def get_card_url(member_id: int) -> str | None:
    ...
    if not (client and bucket and public_url):
        return None
    try:
        client.head_object(Bucket=bucket, Key=_key(member_id))
        return f"{public_url}/{_key(member_id)}"
    except ClientError:
        return None


def upload_card(member_id: int, png_bytes: bytes) -> str | None:
